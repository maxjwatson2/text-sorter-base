...
(deftest main-tests
  (testing "Sanity";; Basic test for sanity. the same as in core tests
    (is (= 1 1)))
  (let[sorting-name  "last-name,Smith,first-name,Adam,email,ab@fake.com,favorite-color,green,birth-date,2/12/1709,
    ...
    (let [main-page (app-routes (-> (mock/request :get "http://localhost:3000/")))
          ...
          returned-recs (app-routes {:request-method :post :uri "/records/add" :query-string (slurp (io/resource "csvnames.txt"))})
